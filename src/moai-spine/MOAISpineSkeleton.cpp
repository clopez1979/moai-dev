...
void MOAISpineSkeleton::UpdateBoundsAndQuads () {
	...
	for ( u32 i = 0; i < size; ++i ) {
        ...
        minX = min ( minX, vertices[VERTEX_X1] );
        minY = min ( minY, vertices[VERTEX_Y1] );
        maxX = max ( maxX, vertices[VERTEX_X1] );
        maxY = max ( maxY, vertices[VERTEX_Y1] );
        minX = min ( minX, vertices[VERTEX_X4] );
        minY = min ( minY, vertices[VERTEX_Y4] );
        maxX = max ( maxX, vertices[VERTEX_X4] );
        maxY = max ( maxY, vertices[VERTEX_Y4] );
        minX = min ( minX, vertices[VERTEX_X2] );
        minY = min ( minY, vertices[VERTEX_Y2] );
        maxX = max ( maxX, vertices[VERTEX_X2] );
        maxY = max ( maxY, vertices[VERTEX_Y2] );
        minX = min ( minX, vertices[VERTEX_X3] );
        minY = min ( minY, vertices[VERTEX_Y3] );
        maxX = max ( maxX, vertices[VERTEX_X3] );
        maxY = max ( maxY, vertices[VERTEX_Y3] );
	}
